...
C_GREY   = '#B1B9C2'   # translated_text (translated_text/translated_text)
C_GREY_L = '#DDE0E7'   # translated_text (translated_text、translated_text)
...
colors_A = [C_ORANGE] + [C_GREY_L] * 9
sns.barplot(data=df_summary.head(10), x='Mean_Test_AUC', y='Model',
...
figA.tight_layout()
figA.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigA_Ranking.pdf"))
figA.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigA_Ranking.png"))
plt.close(figA)
...
sns.boxplot(data=df_folds_top, x='Test_AUC', y='Model', order=top_models,
            color='white', ax=axB, width=0.6,
            boxprops={'linewidth':1.5},
            whiskerprops={'color':'black'}, medianprops={'color':'black'})
boxes = [p for p in axB.get_children() if type(p).__name__ == 'PathPatch']
for i, patch in enumerate(boxes[-10:]):  # translated_text10translated_text10translated_text
    patch.set_edgecolor(C_ORANGE if i == 0 else C_GREY_L)

for i, model in enumerate(top_models):
    subset = df_folds_top[df_folds_top['Model'] == model]
    jitter = np.random.uniform(-0.15, 0.15, size=len(subset))
    color = C_ORANGE if i == 0 else C_GREY_L
    axB.scatter(subset['Test_AUC'], np.full_like(subset['Test_AUC'], i) + jitter,
...
figB.tight_layout()
figB.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigB_Stability.pdf"))
figB.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigB_Stability.png"))
plt.close(figB)
print("  ✅ [B] translated_text")

# ------------------------------------------
# ------------------------------------------
shap_files = glob.glob(os.path.join(data_dir, 'Global_Top20_Features_Cls.csv')) + \
             glob.glob(os.path.join(data_dir, 'Global_Top20_Features.csv'))
if shap_files:
    ...
    cmap_shap = mpl.colors.LinearSegmentedColormap.from_list("shap_gradient", ["#FFFFFF", C_ORANGE])
    ...
    figC.tight_layout()
    figC.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigC_SHAP.pdf"))
    figC.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigC_SHAP.png"))
    plt.close(figC)
...
roc_auc = auc(fpr, tpr)
ax_roc.plot(fpr, tpr, color=C_ORANGE, lw=2.0, label=f'AUC = {roc_auc:.3f}')
ax_roc.plot([0, 1], [0, 1], color=C_GREY, linestyle='--', lw=1.0)
...
cmap_cm = mpl.colors.LinearSegmentedColormap.from_list("cm_custom", [C_GREY_L, C_ORANGE])
sns.heatmap(cm, annot=True, fmt="d", cmap=cmap_cm, cbar=False,
...
figD.tight_layout()
figD.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigD_BestModel_Eval.pdf"))
figD.savefig(os.path.join(OUTPUT_DIR, "UL94_Benchmark_FigD_BestModel_Eval.png"))
plt.close(figD)
print(f"  ✅ [D] translated_text ({best_model_name}) ROC translated_text")

print(f"\n🎉 UL-94 translated_text！translated_text: {OUTPUT_DIR}")